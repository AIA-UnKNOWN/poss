...
// Utils
import { ORDER_CART_ITEMS_KEY, getOrderCartItems } from "src/utils/orderCart.helper";
// Store
...
const useOrderDetailsBar = () => {
  ...
  const [selectedOrderCartItems, setSelectedOrderCartItems] = useState<Product[]>([]);
  ...
  const removeSelectedOrderCartItems = () => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify(localStorageOrderCartItems.filter(
